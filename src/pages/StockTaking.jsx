...
import { printSessionReport } from '../utils/sessionReport';
...
function StockTaking() {
  ...
  // Staff PDF export — the same shared report as the admin Stock overview
  // export, so staff and admin downloads are identical.
  const generateSessionPDF = (session) => {
    const itemsById = Object.fromEntries(allItems.map(i => [i.id, i]));
    printSessionReport(session, itemsById, selectedPub?.name);
  };
